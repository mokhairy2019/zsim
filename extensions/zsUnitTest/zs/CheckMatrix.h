                      ...
                      typename Eigen::MatrixBase<Derived>::Scalar const& tolerance, TestDetails const& details)
{
 bool equal = true;
  ...
  for (index_t i=0; equal && i<rows; ++i)
  {
      ...
      {
          equal &= (zsim::math::abs( expected(i, j) - actual(i, j) )<= tolerance);
      }
  }
...
template< typename T >
void CheckMatrixPartitionOfUnitClose(TestResults& results, zsim::zsMatrix<T> const& actual,
                      T const& tolerance, TestDetails const& details)
{
  ...
  for (index_t j=0; j<cols; ++j)
  {
       ...
       result &= (zsim::math::abs( sum - 1.0) <= tolerance);
  }
