/**
 * @file zsSpaseMatrix.h
 * @brief provides declaration to the sparse matrices in zsim
 * Author : Mohamed Mahmoud
 */

#ifndef ZSIM_ZSSPARSEMATRIX_H
#define ZSIM_ZSSPARSEMATRIX_H
...
/**
   ...
   inserting directly.  Use zsSparseMatrix().setFrom(zsSparseEntries)
...
*/
template<typename T>
class zsSparseEntries : public std::vector<Eigen::Triplet<T,index_t>>
{
public:
...
*/
    ...
    public:
        ...
        zsSparseMatrixIter(const SparseMatrix& mat, const _Index outer)
        :m_outer(outer)
        {
        ...
        inline zsSparseMatrixIter& operator++()
        {
            ++m_values;
            ++m_indices;
            return *this;
 ...
 * Remarks:
 *
 * An entry of the zsSparseMatrix can be accessed by <b>coeff( index
...
// Export the result to a file: saveAsBitmap(...);
    ...
    template<typename T,int _Options,typename _Index>
class zsSparseMatrix : public Eigen::SparseMatrix<T,_Options,_Index>
{
public:
    typedef Eigen::SparseMatrix<T,_Options,_Index> Base;

    typedef zsSparseMatrixIter<T,_Options,_Index> iterator;
    ...
    /// Type defination of block view of sparse matrix
    typedef zsMatrixBlockView<Base> BlockView;

    /// Type pointing to a block view of the sparse matrix
    typedef zsMatrixBlockView<const Base> constBlockView;

    ///shared pointer for zsSparseMatrix
    typedef memory::shared_ptr<zsSparseMatrix> ptr;

    /// Unique pointer for zsSparseMatrix
    typedef memory::unique_ptr<zsSparseMatrix> uPtr;
...
public:
    ...
    zsSparseMatrix(_Index rows, _Index cols) ;

    /// This constructor allows constructing a zsSparseMatrix from Eigen expressions
    template<typename OtherDerived>
    zsSparseMatrix(const Eigen::EigenBase<OtherDerived>& other)  : Base(other) { }

    /// This constructor allows constructing a zsSparseMatrix from a self adjoint view
    template<typename OtherDerived, unsigned int UpLo>
    zsSparseMatrix(const Eigen::SparseSelfAdjointView<OtherDerived, UpLo>& other)
            : Base(other) { }

    /// This constructor allows constructing a zsSparseMatrix from Eigen expressions
    template<typename OtherDerived>
    zsSparseMatrix(const Eigen::MatrixBase<OtherDerived>& other)  : Base(other) { }

    /// This constructor allows constructing a zsSparseMatrix from another sparse expression
    template<typename OtherDerived>
    zsSparseMatrix(const Eigen::SparseMatrixBase<OtherDerived>& other)  : Base(other) { }

    /// This constructor allows constructing a zsSparseMatrix from Eigen expressions
    template<typename OtherDerived>
    zsSparseMatrix(const Eigen::ReturnByValue<OtherDerived>& other)  : Base(other) { }

#if !EIGEN_HAS_RVALUE_REFERENCES
        // swap assignment operator
    zsSparseMatrix & operator=(zsSparseMatrix other)
    {
        this->swap(other);
        return *this;
    }
    ...
    zsSparseMatrix & operator=(const Eigen::SparseSymmetricPermutationProduct<OtherDerived, a>& other)
    {
    ...
    zsSparseMatrix& operator= (const EigenExpr & other)
    {
        this->Base::operator=(other);
        return *this;
    }
        ...
        zsSparseMatrix(const zsSparseMatrix& other) : Base(other)
        { Base::operator=(other); }
        zsSparseMatrix& operator= (const zsSparseMatrix & other)
        { Base::operator=(other); return *this; }

        zsSparseMatrix(zsSparseMatrix&& other)
        { zsSparseMatrix::operator=(std::forward<zsSparseMatrix>(other)); }
...
{
    Base::reserve(zsVector<index_t>::Constant(this->outerSize(), nz));
}

void setFrom( zsSparseEntries<T> const & entries) ;
...
constBlockView blockView(const zsVector<index_t> & rowSizes,
                         const zsVector<index_t> & colSizes) const
{
    return constBlockView(*this, rowSizes, colSizes);
}

/// Returns a pointer wrapped as a zsAsConstVector, which contains
/// the number of non-zero entries per column. Note that the
/// matrix must be uncompressed format for this to work
zsAsConstVector<_Index> nonZerosPerCol()
{
    if ( this->isCompressed() )
    {
        zsWarn<<"nonZerosPerCol(): Uncompressing the zsSparseMatrix.\n";
        this->uncompress();
    }
    return zsAsConstVector<_Index>(this->innerNonZeroPtr(),this->innerSize());
}
...
template<class container>
zsMatrix<T> multiplyBy(const container & inner,
                       const container & outer,
                       const zsMatrix<T> & other)
{
...
zsVector<index_t> nonZerosPerInner(index_t upto = std::numeric_limits<index_t>::max())  const
{
    upto = math::min(upto, this->cols());
    zsVector<index_t> nz(upto);
    index_t * v = nz.data();
...
private:
    ...
    /*
      ...
      M.setZero() is:
      M = zsSparseMatrix(M.rows(), M.cols());
...
//template<class T>
//inline void zsSparseEntries<T>::add( int const& i, int const& j, const T & value)
//        { this->push_back( Triplet(i,j,value) ); }


template<typename T, int _Options, typename _Index> inline
zsSparseMatrix<T, _Options, _Index>::zsSparseMatrix() : Base() { }

template<typename T, int _Options, typename _Index> inline
zsSparseMatrix<T, _Options, _Index>::zsSparseMatrix(_Index rows, _Index cols) : Base(rows,cols) { }

template<typename T, int _Options, typename _Index> inline
void zsSparseMatrix<T, _Options, _Index>::setFrom( zsSparseEntries<T> const & entries)
{ this->setFromTriplets(entries.begin(),entries.end() ); }

template<typename T, int _Options, typename _Index> void
zsSparseMatrix<T, _Options, _Index>::rrefInPlace()
{
    zsMatrix<T,1,Dynamic> R;
    index_t c_i, c_j;
    const index_t nr = this->rows();
    const index_t nc = this->cols();

    zsVector<index_t> piv(nr);
...
namespace Eigen { namespace internal {
        template<typename T, int _Options, typename _Index>
        struct traits<zsim::zsSparseMatrix<T,_Options,_Index> >:
        Eigen::internal::traits<Eigen::SparseMatrix<T,_Options,_Index> > { };
} }

#endif //ZSIM_ZSSPARSEMATRIX_H
