...
#ifndef ZSIM_ZSTIMER_H
#define ZSIM_ZSTIMER_H
...
/** @brief A Stopwatch object can be used to measure execution time of
    ...
    - For wall time use the  \ref zsStopwatch typedef.

    - For CPU time use the \ref zsCPUStopwatch typedef.
...
*/
    ...
    class zsGenericStopwatch
    {
    public:

        /// Declares a stop-watch
        zsGenericStopwatch() : m_value(0) { restart(); }
        ...
        friend std::ostream& operator<< (std::ostream& os, const zsGenericStopwatch& sw)
        { return formatTime(os, sw.m_value); }

    private:
        zsGenericStopwatch (const zsGenericStopwatch&);
        zsGenericStopwatch& operator=(const zsGenericStopwatch&);
...
/// @brief A stop-watch measuring real (wall) time
    typedef zsGenericStopwatch<CXX11WallClock> zsStopwatch;
...
/// @brief A stop-watch measuring real (wall) time
typedef zsGenericStopwatch<LinuxWallClock> zsStopwatch;
...
/// @brief A stop-watch measuring real (wall) time
typedef zsGenericStopwatch<WindowsWallClock> zsStopwatch;
...
/// @brief A stop-watch measuring real (wall) time
typedef zsGenericStopwatch<WallClock> zsStopwatch;
...
/// @brief A stop-watch measuring CPU time
    typedef zsGenericStopwatch<LinuxCPUClock> zsCPUStopwatch;
...
/// @brief A stop-watch measuring CPU time
typedef zsGenericStopwatch<CPUClock> zsCPUStopwatch;

#endif
    class zsEngngModelTimer {

    };
}
#endif //ZSIM_ZSTIMER_H
