...
//
//
//    void zsEngngModel :: setParallelMode(bool newParallelFlag)
//    {
...
//
//
//    int zsEngngModel :: instanciateYourself(DataReader &dr, InputRecord &ir, const char *dataOutputFileName, const char *desc)
//    {
...
//
//#  ifdef VERBOSE
//        ZSIM_LOG_DEBUG( "Reading all data from \"%s\"\n", referenceFileName.c_str() );
//#  endif
//
...
//            ZSIM_ERROR("Error initializing from user input: %s\n", e.what());
//        }
//
//        return 1;
//    }
...
//
//    void
//    zsEngngModel :: initializeFrom(InputRecord &ir)
//    {
//        IR_GIVE_FIELD(ir, numberOfSteps, _IFT_EngngModel_nsteps);
//        if ( numberOfSteps <= 0 ) {
//            ZSIM_ERROR("nsteps not specified, bad format");
//        }
//
//        contextOutputStep =  0;
...
//                ZSIM_ERROR("Can't open output file %s", this->dataOutputFileName.c_str());
//            }
//
...
//
//    int
//    zsEngngModel :: instanciateDomains(DataReader &dr)
//    {
//        int result = 1;
...
//
//    int
//    zsEngngModel :: instanciateMetaSteps(DataReader &dr)
//    {
//        // create meta steps
//        metaStepList.clear();
//        metaStepList.reserve(nMetaSteps);
//        for ( int i = 1; i <= this->nMetaSteps; i++ ) {
//            //zsMetaStep *mstep = new zsMetaStep(i, this);
...
//
//    int
//    zsEngngModel :: instanciateDefaultMetaStep(InputRecord &ir)
//    {
//        if ( numberOfSteps == 0 ) {
//            ZSIM_ERROR("nsteps cannot be zero");
//        }
//
//        // create default meta steps
//        this->nMetaSteps = 1;
//        metaStepList.clear();
//        //zsMetaStep *mstep = new zsMetaStep(1, this, numberOfSteps, *ir);
...
//
//    int
//    zsEngngModel :: giveNumberOfDomainEquations(int id, const UnknownNumberingScheme &num)
//    {
//        //
...
//
//    int
//    zsEngngModel :: forceEquationNumbering(int id)
//    {
...
//            timer.stopTimer();
//
//            ZSIM_LOG_DEBUG( "Sloan's algorithm done in %.2fs\n", timer.getUtime() );
//            ZSIM_LOG_DEBUG("Nominal profile %d (old) %d (new)\n", initialProfile, optimalProfile);
//
...
//                ZSIM_LOG_INFO("zsEngngModel info: user time consumed by solution step %d: %.2fs\n",
...
//    }
//
//    zsTimeStep* zsEngngModel :: generateNextStep()
//    {
//        int smstep = 1, sjstep = 1;
//        if ( this->currentStep ) {
//            smstep = this->currentStep->giveMetaStepNumber();
...
//        }
//
//        // test if sjstep still valid for zsMetaStep
...
//
//    void
//    zsEngngModel :: initMetaStepAttributes(MetaStep *mStep)
//    {
//        // update attributes
...
//
//    void
//    zsEngngModel :: updateAttributes(MetaStep *mStep)
//    {
...
//
//    void
//    zsEngngModel :: updateYourself(TimeStep *tStep)
//    {
//        for ( auto &domain: domainList ) {
//#  ifdef VERBOSE
...
//
//    void
//    zsEngngModel :: terminate(TimeStep *tStep)
//    {
//        if ( !suppressOutput ) {
...
//
//    void
//    zsEngngModel :: doStepOutput(TimeStep *tStep)
//    {
//        if ( !suppressOutput ) {
...
//
//    void
//    zsEngngModel :: saveStepContext(TimeStep *tStep, ContextMode mode)
//    {
...
//
//    void
//    zsEngngModel :: printOutputAt(FILE *file, TimeStep *tStep)
//    {
//        int domCount = 0;
//
//        for ( auto &domain: domainList ) {
...
//
//    void
//    zsEngngModel :: printOutputAt(FILE *file, TimeStep *tStep, const zsIntArray &nodeSets, const zsIntArray &elementSets)
//    {
//        for ( auto &domain: domainList ) {
...
//
//    void
//    zsEngngModel :: outputNodes(FILE *file, Domain &domain, TimeStep *tStep, int setNum)
//    {
//        fprintf(file, "\n\nNode output:\n------------------\n");
//
...
//
//    void
//    zsEngngModel :: outputElements(FILE *file, Domain &domain, TimeStep *tStep, int setNum)
//    {
//        fprintf(file, "\n\nElement output:\n---------------\n");
//
//        if ( setNum == 0 ) {
...
//
//
//    void zsEngngModel :: printYourself()
//    {
...
//    }
//
//    void zsEngngModel :: printDofOutputAt(FILE *stream, Dof *iDof, TimeStep *tStep)
//    {
//        iDof->printSingleOutputAt(stream, tStep, 'd', VM_Total);
//    }
//
//    void zsEngngModel :: assemble(SparseMtrx &answer, TimeStep *tStep, const MatrixAssembler &ma,
...
//                    ZSIM_ERROR("sparse matrix assemble error");
//                }
//            }
//        }
//
...
//                    const zsIntArray &elements = set->giveElementList();
...
//            ZSIM_ERROR("Contact problems temporarily deactivated");
...
//    {
//        zsIntArray r_loc, c_loc, dofids(0);
//        FloatMatrix mat, R;
//
//        this->timer.resumeTimer(EngngModelTimer :: EMTT_NetComputationalStepTimer);
//        int nelem = domain->giveNumberOfElements();
...
//
//        if ( domain->hasContactManager() ) {
//            ZSIM_ERROR("Contant problems temporarily deactivated");
...
//
//
//    void zsEngngModel :: assembleVector(FloatArray &answer, TimeStep *tStep,
...
//
//
//    void zsEngngModel :: assembleVectorFromDofManagers(FloatArray &answer, TimeStep *tStep, const VectorAssembler &va, ValueModeType mode,
//                                                       const UnknownNumberingScheme &s, Domain *domain, FloatArray *eNorms)
//    {
//        ///@todo This should be removed when loads are given through sets.
//        zsIntArray loc, dofids;
//        FloatArray charVec;
//        FloatMatrix R;
//        zsIntArray dofIDarry;
...
//
//
//    void zsEngngModel :: assembleVectorFromBC(FloatArray &answer, TimeStep *tStep,
//                                              const VectorAssembler &va, ValueModeType mode,
//                                              const UnknownNumberingScheme &s, Domain *domain, FloatArray *eNorms)
//    {
...
//                    const zsIntArray &boundaries = set->giveBoundaryList();
...
//                    const zsIntArray &edgeBoundaries = set->giveEdgeList();
...
//                    const zsIntArray &nodes = set->giveNodeList();
...
//
//
//    void zsEngngModel :: assembleVectorFromElements(FloatArray &answer, TimeStep *tStep,
//                                                    const VectorAssembler &va, ValueModeType mode,
//                                                    const UnknownNumberingScheme &s, Domain *domain, FloatArray *eNorms)
////
...
//            const zsIntArray& list = element->giveBodyLoadList();
//            if (!list.isEmpty()) {
...
//            const zsIntArray& list2 = element->giveBoundaryLoadList();
//
...
//                    }
//                } else {
//                    ZSIM_ERROR ("Unsupported element boundary load type");
//                }
//
//                if ( assembleFlag ) {
...
//
//    void
//    zsEngngModel :: assembleExtrapolatedForces(FloatArray &answer, TimeStep *tStep, zsCharType type, Domain *domain)
//    {
...
//
//    void
//    zsEngngModel :: assemblePrescribedExtrapolatedForces(FloatArray &answer, TimeStep *tStep, zsCharType type, Domain *domain)
//    {
//        // Simply assembles contributions from each element in domain
//        zsIntArray loc;
//        FloatArray charVec, delta_u;
...
//
//    void
//    zsEngngModel :: assembleVectorFromContacts(FloatArray &answer, TimeStep *tStep, zsCharType type, ValueModeType mode,
//                                               const UnknownNumberingScheme &s, Domain *domain, FloatArray *eNorms)
//    {
//        if( domain->hasContactManager()) {
...
//
//    void
//    zsEngngModel :: updateComponent(TimeStep *tStep, zsNumericalCmpn cmpn, Domain *d)
//    {
//        ZSIM_ERROR("Unknown Type of component.");
//    }
//
//
//    void
//    zsEngngModel :: updateSolution(FloatArray &solutionVector, TimeStep *tStep, Domain *d)
//    {
//        ZSIM_ERROR("updateSolution is not implemented.");
//    }
//
//
//    void
//    zsEngngModel :: updateInternalRHS(FloatArray &answer, TimeStep *tStep, Domain *d, FloatArray *eNorms)
//    {
//        ZSIM_ERROR("updateInternalRHS is not implemented.");
//    }
//
//
//    void
//    zsEngngModel :: updateMatrix(SparseMtrx &mat, TimeStep *tStep, Domain *d)
//    {
//        ZSIM_ERROR("updateMatrix is not implemented.");
//    }
//
//
//    void
//    zsEngngModel :: initStepIncrements()
////
...
//
//    void
//    zsEngngModel :: updateDomainLinks()
//    {
//        this->giveExportModuleManager()->initialize();
//    }
//
//
//    void zsEngngModel :: saveContext(DataStream &stream, ContextMode mode)
////
...
////
//// - save zsEngngModel state varialbles as displacement, velocity, .. vectors
...
//
//
//    void zsEngngModel :: restoreContext(DataStream &stream, ContextMode mode)
////
//// this procedure is used mainly for two reasons:
////
...
////
//// - rest. zsEngngModel state varialbles as displacement, velocity, .. vectors
...
//
//    zsMetaStep *
//    zsEngngModel :: giveCurrentMetaStep()
//    {
...
//
//    std ::string
//    zsEngngModel :: giveContextFileName(int tStepNumber, int stepVersion) const
//    {
...
//
//    std :: string
//    zsEngngModel :: giveDomainFileName(int domainNum, int domainSerNum) const
//    {
//        std :: string fname = this->coreOutputFileName;
//        char fext [ 100 ];
...
//
//    std :: string
//    zsEngngModel :: errorInfo(const char *func) const
//    {
//        if ( this->isParallel() ) {
...
//
//    void
//    zsEngngModel :: setDomain(int i, Domain *ptr, bool iDeallocateOld)
//    {
//        if ( i < 1 || i > (int)this->domainList.size() ) {
//            ZSIM_ERROR("zsDomain index %d out of range [1,%d]", i, (int)this->domainList.size());
//        }
//        if ( !iDeallocateOld ) {
...
//
//    zsParallelContext *
//    zsEngngModel :: giveParallelContext(int i)
//    {
//        if  ( i > (int)parallelContextList.size() ) {
//            ZSIM_ERROR("context not initialized for this problem");
//        }
//
...
//
//    void
//    zsEngngModel :: initParallelContexts()
//    {
...
//
//    zsMetaStep *
//    zsEngngModel :: giveMetaStep(int i)
//    {
//        if ( ( i > 0 ) && ( i <= this->nMetaSteps ) ) {
//            return &this->metaStepList[i-1];
//        } else {
//            ZSIM_ERROR("undefined metaStep (%d)", i);
//        }
//
//        return NULL;
//    }
//
//    void
//    zsEngngModel :: letOutputBaseFileNameBe(const std :: string &src)
//    {
...
//
//    FILE *
//    zsEngngModel :: giveOutputStream()
//// Returns an output stream on the data file of the receiver.
//    {
//        if ( !outputStream ) {
//            ZSIM_ERROR("No output stream opened!");
//        }
//
//        return outputStream;
//    }
//
//    double
//    zsEngngModel :: giveSolutionStepTime()
//    {
//        return this->timer.getUtime(EngngModelTimer :: EMTT_SolutionStepTimer);
//    }
//
//    void
//    zsEngngModel :: giveAnalysisTime(int &rhrs, int &rmin, int &rsec, int &uhrs, int &umin, int &usec)
//    {
...
//
//    void
//    zsEngngModel :: terminateAnalysis()
//    {
...
//
//        ZSIM_LOG_FORCED("\n\nANALYSIS FINISHED\n\n\n");
//        ZSIM_LOG_FORCED("Real time consumed: %03dh:%02dm:%02ds\n", rhrs, rmin, rsec);
//        ZSIM_LOG_FORCED("User time consumed: %03dh:%02dm:%02ds\n", uhrs, umin, usec);
//        exportModuleManager.terminate();
//    }
//
//    int
//    zsEngngModel :: checkProblemConsistency()
//    {
//        int result = 1;
//
...
//#  ifdef VERBOSE
//        if ( result ) {
//        ZSIM_LOG_DEBUG("Consistency check:  OK\n");
//    } else {
...
//#else
//        ZSIM_ERROR("Can't do it, only compiled for sequential runs");
//#endif
//#ifdef __VERBOSE_PARALLEL
//        ZSIM_LOG_RELEVANT("[%d/%d] Running on %s\n", rank, numProcs, processor_name);
//#endif
//    }
//
//
//#ifdef __OOFEG
//    void zsEngngModel :: drawYourself(zsoofegGraphicContext &gc)
//{
...
//}
//
//void zsEngngModel :: drawElements(zsoofegGraphicContext &gc)
//{
//    Domain *d = this->giveDomain( gc.getActiveDomain() );
//    zsTimeStep *tStep = this->giveCurrentStep();
//    for ( auto &elem : d->giveElements() ) {
//        elem->drawYourself(gc, tStep);
//    }
//}
//
//void zsEngngModel :: drawNodes(zsoofegGraphicContext &gc)
//{
//    Domain *d = this->giveDomain( gc.getActiveDomain() );
//    zsTimeStep *tStep = this->giveCurrentStep();
...
//
//    void
//    zsEngngModel :: initializeCommMaps(bool forceInit)
//    {
//#ifdef __PARALLEL_MODE
...
//    }
//#else
//        ZSIM_ERROR("Can't set up comm maps, parallel support not compiled");
//#endif
//    }
//
//
//    int
//    zsEngngModel :: updateSharedDofManagers(FloatArray &answer, const UnknownNumberingScheme &s, int ExchangeTag)
//    {
//        if ( isParallel() ) {
//#ifdef __PARALLEL_MODE
//            int result = 1;
// #ifdef __VERBOSE_PARALLEL
//        VERBOSEPARALLEL_PRINT( "zsEngngModel :: updateSharedDofManagers", "Packing data", this->giveRank() );
// #endif
//
//        ArrayWithNumbering tmp;
//        tmp.array = & answer;
//        tmp.numbering = & s;
//        result &= communicator->packAllData(this, & tmp, & zsEngngModel :: packDofManagers);
//
// #ifdef __VERBOSE_PARALLEL
//        VERBOSEPARALLEL_PRINT( "zsEngngModel :: updateSharedDofManagers", "Exchange started", this->giveRank() );
// #endif
//
//        result &= communicator->initExchange(ExchangeTag);
//
// #ifdef __VERBOSE_PARALLEL
//        VERBOSEPARALLEL_PRINT( "zsEngngModel :: updateSharedDofManagers", "Receiving and unpacking", this->giveRank() );
// #endif
//
//        result &= communicator->unpackAllData(this, & tmp, & zsEngngModel :: unpackDofManagers);
//        result &= communicator->finishExchange();
//        return result;
//#else
//            ZSIM_ERROR("Support for parallel mode not compiled in.");
//            return 0;
//#endif
//        } else {
//            return 1;
...
//
//    int
//    zsEngngModel :: exchangeRemoteElementData(int ExchangeTag)
//    {
//
...
//            int result = 1;
// #ifdef __VERBOSE_PARALLEL
//        VERBOSEPARALLEL_PRINT( "zsEngngModel :: exchangeRemoteElementData", "Packing remote element data", this->giveRank() );
// #endif
//
//        result &= nonlocCommunicator->packAllData(this, & zsEngngModel :: packRemoteElementData);
//
// #ifdef __VERBOSE_PARALLEL
//        VERBOSEPARALLEL_PRINT( "zsEngngModel :: exchangeRemoteElementData", "Remote element data exchange started", this->giveRank() );
// #endif
//
//        result &= nonlocCommunicator->initExchange(ExchangeTag);
//
// #ifdef __VERBOSE_PARALLEL
//        VERBOSEPARALLEL_PRINT( "zsEngngModel :: exchangeRemoteElementData", "Receiveng and Unpacking remote element data", this->giveRank() );
// #endif
//
//        if ( !( result &= nonlocCommunicator->unpackAllData(this, & zsEngngModel :: unpackRemoteElementData) ) ) {
//            ZSIM_ERROR("Receiveng and Unpacking remote element data");
//        }
//
...
//#ifdef __PARALLEL_MODE
//    void
//zsEngngModel :: balanceLoad(zsTimeStep *tStep)
//{
//    this->giveLoadBalancerMonitor();
//    this->giveLoadBalancer();
//    if ( !lb ) {
//        ZSIM_WARNING("No load balancer found, skipping load balancing step");
//        return;
//    }
//
...
//        zsLoadBalancerMonitor :: LoadBalancerDecisionType _d = lbm->decide(tStep);
//        if ( ( _d == zsLoadBalancerMonitor :: LBD_RECOVER ) ||
...
//                }
//
//                for ( zsDof *dof: *giveDomain(1)->giveDofManager(i) ) {
...
//            ZSIM_LOG_INFO("[%d] zsEngngModel info: user time consumed by load rebalancing %.2fs\n",
...
//
//int
//zsEngngModel :: packRemoteElementData(zsProcessCommunicator &processComm)
//{
//    int result = 1;
...
//
//int
//zsEngngModel :: unpackRemoteElementData(zsProcessCommunicator &processComm)
//{
//    int result = 1;
...
//        } else {
//            ZSIM_ERROR("element is not remote");
//        }
//    }
//
//    return result;
...
//
//int
//zsEngngModel :: packDofManagers(ArrayWithNumbering *srcData, zsProcessCommunicator &processComm)
//{
//    zsFloatArray *src = srcData->array;
//    const zsUnknownNumberingScheme &s = * srcData->numbering;
//    int result = 1;
//    Domain *domain = this->giveDomain(1);
//    const IntArray &toSendMap = processComm.giveToSendMap();
//    zsProcessCommunicatorBuff *pcbuff = processComm.giveProcessCommunicatorBuff();
//
//    ///@todo Shouldn't hardcode domain number 1
//    ///@todo Must fix: Internal dofmanagers in xfem and bc
//    for ( int inode : toSendMap ) {
//        zsDofManager *dman = domain->giveDofManager( inode );
//        for ( auto &jdof: *dman ) {
...
//
//int
//zsEngngModel :: unpackDofManagers(ArrayWithNumbering *destData, zsProcessCommunicator &processComm)
//{
//    zsFloatArray *dest = destData->array;
//    const zsUnknownNumberingScheme &s = * destData->numbering;
//    int result = 1;
//    Domain *domain = this->giveDomain(1);
//    const IntArray &toRecvMap = processComm.giveToRecvMap();
//    zsProcessCommunicatorBuff *pcbuff = processComm.giveProcessCommunicatorBuff();
...
//                    dest->at(eqNum)  = value;
//                } else {
//                    ZSIM_ERROR("unknown dof namager parallel mode");
//                }
//            }
//        }
//    }
...
// *
// *
// *             ZSIM : Object Oriented Finite Element Code
// *
