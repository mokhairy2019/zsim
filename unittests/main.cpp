...
/** @file main.cpp
    ...
    For a reference on creating tests, see zsTutorial.cpp
...
namespace zsim {
    ...
    public:
        zsUnitTestSelector(int argc, char* argv[])
        ...
        bool operator()(const UnitTest::Test * const testCase) const
        {
            ...
            for (int i=1; i<m_argc; ++i)
            {
                ...
                //TODO: Reactivate after creating zsFileManager
                //toRun |= zsFileManager::pathEqual(testCase->m_details.filename, m_argv[i]);// exact match up to path sep.
            }
            m_did_run |= toRun;
            return toRun;
        }
...
{
    //TODO: Reactivate after creating the function printVersion
    //zsCmdLine::printVersion();

    if (argc > 1)
    {
        UnitTest::TestReporterStdout reporter;
        UnitTest::TestRunner runner(reporter);
        zsUnitTestSelector sel(argc,argv);
        int result = runner.RunTestsIf(UnitTest::Test::GetTestList(), NULL, sel, 0);
        if (!sel.didRunAnyTests())
        {
            zsInfo << "Did not find any matching test.\n";
            return 1;
        }
        return result;
    }
